...
func matchLines(content []byte, query string, tags func() (string, string), visit func(searchResultLine)) {
	...
	for len(buf) > 0 {
		...
		if i+offset < len(buf) {
			buf = buf[i+offset:] // needed to fix out of bound error prior caused by syntax hightlighting with chroma
		} else {
			buf = nil
		}
